...
int record_db_info(mysql_session* sess, char cmd, int msg_len, char* msg){
    ...
    if(msg_len >= MYSQL_DB_NAME_LEN){
        memcpy(sess->switch_dbname, msg, MYSQL_DB_NAME_LEN - 1);
        sess->switch_dbname[MYSQL_DB_NAME_LEN - 1] = '\0';
        snprintf(info->data, info->alloc_size, "use %s", sess->switch_dbname);
        info->count = strlen(info->data);
    }else{
        memcpy(sess->switch_dbname, msg, msg_len);
        sess->switch_dbname[msg_len] = '\0';
        snprintf(info->data, info->alloc_size, "use %s", sess->switch_dbname);
        info->count = strlen(info->data);
    }
    return 0;
...
int mysql_dissect_query_result(mysql_session* sess, half_stream* stream){
    ...
    sess->query_info->response_packet_type = 1; //default: not Error packet
...
int handle_server_msg(half_stream* stream, mysql_session* sess){
    ...
    if(sess->state == SESSION_STATE_QUERY){
        if(/* ret != SESSION_IGNORE_WRONG_SYNTAX &&*/ sess->query_info->data[0] != '\0'){
            // 修复数据库名切换失败时显示错误的bug
            if (sess->switch_dbname[0] != '\0') {
                if (sess->query_info->response_packet_type == 1) {
                    memcpy(sess->dbname, sess->switch_dbname, MYSQL_DB_NAME_LEN - 1);
                    sess->dbname[MYSQL_DB_NAME_LEN - 1] = '\0';
                } else {
                    sess->switch_dbname[0] = '\0';
                }
