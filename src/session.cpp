...
mysql_session* mysql_session_new(){
    ...
    new_session->switch_dbname = (char*)malloc(MYSQL_DB_NAME_LEN);
    new_session->switch_dbname[0] = '\0';
...
void mysql_session_free(mysql_session* sess){
    if(sess != NULL){
        ...
        if(sess->switch_dbname != NULL){
            free(sess->switch_dbname);
        }
...
mysql_session* add_mysql_session(session_key_t* skey){
    ...
    sess->skey = *skey;
    memcpy(sess->other_info.client_mac, g_mac + 6, 6);
    memcpy(sess->other_info.server_mac, g_mac, 6);
