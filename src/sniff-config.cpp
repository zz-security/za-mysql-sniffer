...
static void print_help(const char* argv0){
    fprintf(stderr,
            ...
            "         -T tcp server addr, -T 127.0.0.1:8090, the query log would be output to tcp server\n",
            argv0);
}
...
static int parse_cmdline_option(int argc, char** argv){
    ...
    int opt_len;
    string whitelist;
    while((opt = getopt(argc, argv, "dhi:s:p:l:e:f:r:w:t:n:T:")) != -1 && ret != -1){
        switch(opt){
            case 'd':
                config.daemon = 1;
                break;
            ...
            case 'n':
                ...
                break;
            case 'T':
                config.output_to_tcp_server = 1;
                config.tcp_server_addr = optarg;
                break;
            case 'h':
            default:
                print_help(argv[0]);
...
int config_init(int argc, char** argv){
    ...
    config.output_to_tcp_server = 0;
    config.tcp_server_addr = "";
...
int config_is_output_to_tcp_server() {
    return config.output_to_tcp_server;
}

const char* config_get_tcp_server_addr() {
    return config.tcp_server_addr.c_str();
}
